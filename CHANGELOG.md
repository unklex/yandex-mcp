...
- **Настоящий структурированный вывод вместо строки для повторного парсинга.**
  Все 30 инструментов (`tools/*.py`) раньше возвращали `-> str` — заранее
  сериализованную через `json.dumps(..., ensure_ascii=False, indent=2)` строку.
  С mcp 2.x это автоматически оборачивалось в `structuredContent.result`
  (строка внутри строки — клиенту всё равно приходилось её парсить). Теперь
  каждый инструмент возвращает `-> dict[str, Any]` напрямую, и SDK сам строит:
  - `content[0].text` — **байт-в-байт идентичен** прежнему выводу (проверено
    сравнением текста content и сериализации `structuredContent` для
    `get_realtime` и `get_direct_budget` — совпадают полностью, включая live-ответы
    от Metrika и Direct API);
  - `structuredContent` — теперь настоящий вложенный объект (`date`,
    `counter_id`, `totals: {...}`, `by_hour: [...]`, ...), а не строка;
    клиенты могут читать поля напрямую, без повторного `JSON.parse`;
  - `outputSchema` в `tools/list` — `{"type": "object", "additionalProperties": true}`
    вместо прежней обёртки `{"result": {"type": "string"}}`.
- Побочный эффект: ответы об ошибках (`{"error": "..."}"`) теперь тоже всегда
  красиво отформатированы (`indent=2`), раньше были в одну строку — только
  форматирование текста, структура данных не менялась.
- Модульные хелперы `_no_direct_error()` / `_no_client_error()` (возвращали
  готовую JSON-строку с ошибкой) теперь тоже возвращают `dict[str, Any]`.
- `import json` убран из всех 14 модулей `tools/*.py` — сериализация теперь
  полностью на стороне SDK.
- Верифицировано: все 3 000+ строковых литерала во всех 14 файлах сверены
  по AST до/после правки (множество совпадает 1:1 в каждом файле) — правка
  не могла случайно обрезать или исказить текст (в первой попытке скрипта
  была найдена и исправлена ошибка: `ast`-модуль отдаёт `col_offset` в
  UTF-8-байтах, а не в символах, что на кириллических строках дало смещения).
- Имена инструментов, параметры и логическая форма JSON-ответа — без изменений.
...
**`tools/wordstat.py`** — исследование ключевых фраз. Изначально реализовано
через legacy Direct API v4 (`CreateNewWordstatReport` + polling); позже
(без отдельной записи в этом changelog) переписано на Yandex Cloud Search API v2
— синхронный REST одним POST, ключ сервисного аккаунта вместо OAuth/Client-Login,
и рабочая помесячная динамика (v4 перестал отдавать `MonthList`). Актуальное
описание — в docstring `tools/wordstat.py`. Инструменты не переименовывались:
- `wordstat_top_requests` — топ смежных запросов по 1–10 фразам, с фильтром по региону
